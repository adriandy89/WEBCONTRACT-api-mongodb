...
func EntityRoutes(r *mux.Router) {
	...
	r.HandleFunc("/api/entity/{codeCompany}", middlewares.CheckDB(middlewares.ValidateJWT(entitycontroller.GetEntityPrefixByCodeCompany))).Methods("GET")
	r.HandleFunc("/api/entity/{codeCompany}/full", middlewares.CheckDB(middlewares.ValidateJWT(entitycontroller.GetEntityByCodeCompany))).Methods("GET")
